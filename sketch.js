...
function preload() {
  ...
  fontBold = loadFont(
    "./fonts/Source_Serif_4/static/SourceSerif4-SemiBold.ttf"
  );
...
function drawHelper() {
  ...
  drawDescription(xloc, yloc + textSize() / 2, rectWidth, rectHeight);
}
...
function drawDescription(xloc, yloc, wdth, hght) {
  ...
  fill(blackColor);
  let description =
    "This data portrait analyses the linguistic intricacies of Cassandra Clare's bestselling novel, The Clockwork Angel. It extracts and visualizes the frequency of every word used in the novel in addition to classifying the word according to it's part of speech.";
  let legend = [
    "Word Frequency",
    "Less than " + countRange[0],
    "Between " + countRange[0] + " & " + countRange[1],
    "Between " + countRange[1] + " & " + countRange[2],
    "Between " + countRange[2] + " & " + countRange[3],
    "Greater than " + countRange[3],
  ];
  let finalX =
    posTextInfo[posTextInfo.length - 1].xpos +
    posTextInfo[posTextInfo.length - 1].txtWidth;
  ...
  text(
    description,
    xloc,
    yloc * 2 + hght + textSize() + descriptionLineSpacing,
    descriptionTxtBoxWidth
  );
  ...
  for (let i = 0; i < palette.length; i++) {
    fill(palette[i]);
    rect(
      finalX - legendRectWidth / 2,
      curr + descriptionLineSpacing,
      legendRectWidth / 2,
      textSize() - descriptionLineSpacing / 2
    );
    curr += textSize();
  }
}
...
function dataSetUp() {
  ...
  for (word in dataset) {
    if (
      ...
      word.includes("1878") ||
      dataset[word].pos === "X" ||
      !Object.keys(wordList).includes(dataset[word].pos)
    ) {
      continue;
    }
...
function partOfSpeechDrawHelper(startingX, startingY) {
  ...
  for (let i = 0; i < posTextInfo.length; i++) {
    ...
    rect(
      posTextInfo[i].xpos,
      posTextInfo[i].ypos,
      posTextInfo[i].txtWidth,
      posTextInfo[i].txtHeight
    );
    fill(txtColor);
...
function checkMouseClicked(obj) {
  if (
    mouseIsPressed &&
    mouseX > obj.xpos &&
    mouseX < obj.xpos + obj.txtWidth &&
    mouseY > obj.ypos &&
    mouseY < obj.ypos + obj.txtHeight
  ) {
    obj.selected = true;
    displayID = obj.id;
  }
